...
func (p *GlesysProvider) submitChanges(ctx context.Context, changes *glesysChanges) error {
	...
	for _, create := range changes.Creates {
		...
		if !p.DryRun {
			_, err := p.Client.DNSDomains.AddRecord(ctx, *create)
			if err != nil {
				return err
			}
		}
	...
	for _, updates := range changes.Updates {
		if !p.DryRun {
			_, err := p.Client.DNSDomains.UpdateRecord(ctx, *updates)
			if err != nil {
				return err
			}
		}
	...
	for _, deletes := range changes.Deletes {
		if !p.DryRun {
			err := p.Client.DNSDomains.DeleteRecord(ctx, deletes.RecordID)
			if err != nil {
				return err
			}
		}
